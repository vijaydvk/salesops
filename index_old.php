	...
	if ($user = K18_user::checkuserid( $_POST['user_id'] , $_POST['user_pwd']))
	{
		...
		if ( $user->checkPassword( $_POST['user_pwd'] ) ) {
			...
			//{
				...
				// The following lines were moved out of if else condition - binay request
...
function saveUsers()
{
	// Kuppuram add try - catch block - 30-11-2017
	try
	{
		$results = array();
...

{
	...
	require( TEMPLATE_PATH . "/settings_views_page1.php" );
...

{
...

	{
		...
		settings_op::update_oreder_prod();
...

	{
		...
		settings_op::delete_oreder_prod();
...

	{
		...
		settings_op::activate_oreder_prod();
